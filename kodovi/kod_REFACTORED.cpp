...
// REGISTRI ZA DISPLAY - preuzeto sa datasheet-a i linka: https://os.mbed.com/users/andcor02/code/max7219ledarray/rev/
#define NO_OP_REG           0x00    // no operation registar
...
#define DECODE_MODE_REG     0x09    // oznacava da li se koristi decode mode
#define INTENSITY_REG       0x0A    // intenzitet kojim diode svijetle
...
// postavljanje defaultnih vrijednosti za sve 4 matrice (initialization routine - koristi se nakon što se sistem tek upali)
void init()
{
    for(int i = 1; i <= 4; ++i)
    {
        pisi(DECODE_MODE_REG, 0x00, i);    // decode mode ne koristimo
        pisi(INTENSITY_REG, 0x0f, i);      // stavi duty cycle na maksimalnu mogucu vrijednost (osvijetljenost)
        ...
        pisi(0x0F, 0x0F, i);               // udji u display test mode
        wait_ms(500);                      // radi 500 ms
        ocistiDisplej(i);                  // ugasi sve
        pisi(0x0F, 0x00, i);               // izadji iz display test mode
        wait_ms(500);                      // cekaj 500ms
    }
}
...
// iscrtava trenutnu konfiguraciju ploce na display
void osvjezi_displej()
{
    unsigned char i = 0, j = 0, c = 0;
    for(; i < 8; ++i) // gornja polovina
    {
        for(j = 0; j < 8; ++j)   // prva matrica (gore-lijevo)
            red[j] = trenutno[i][j];

        c = spoji_red(red);
        pisi(i+1,c,1);

        for(j = 8; j < 16; ++j) // druga matrica (gore-desno)
            red[j-8] = trenutno[i][j];
    ...
    for(; i < 16; ++i) // donja polovina
    {
        ...
        for(j = 0; j < 8; ++j) // treca matrica (dole-lijevo)
            red[j] = trenutno[i][j];

        c = spoji_red(red);
        pisi(i-7,c,3);

        for(j = 8; j < 16; ++j) // cetvrta matrica (dole-desno)
            red[j-8] = trenutno[i][j];
...
// racuna broj zivih komsija celije
...
void threadGameOfLife()
{
    while(1) // vrti beskonacno update
    {
       ...
       if(pauza == 1) continue; // ako je igra pauzirana, ne radi nista
       update_game(); // radi update
    }
}

// postavlja boju teksta u putty na zelenu (koju korisitmo za obavjestenja)
void zelena()
{
 pc.printf("\033[0;32m");
}

// postavlja boju teksta u putty na crvenu (koju koristimo za upozorenja)
void crvena()
{
    pc.printf("\033[0;31m");
}

// vraca boju teksta na pocetnu
void defaultna()
{
 pc.printf("\033[0m");
}

// ispisuje informacije o kreatorima
void credits()
{
    zelena();
    pc.printf("\r\n===============================================================\r\n");
...
// generise pseudonasumicnu konfiguraciju celija i iscrtava to na display
void nasumicno()
{
    int i  = 0, j = 0;
    for(i = 0; i<16;++i)
...
// ubrzava mijenjanje generacija za 0.1 sec (max brzina = 0.1sec)
void ubrzaj()
{
    vrijeme_cekanja-=0.1;
...
// usporava mijenjanje generacija za 0.1 sec (min brzina = 1.0sec)
void uspori()
{
    vrijeme_cekanja+=0.1;
...
// provjerava da li je karakter cifra
int isNum(char c)
{
    int a = c-48;
    return (a >= 0 && a < 9) ? 1 : 0;
...
// otvara korisniku dijalog za unosenje koordinata diode koju mijenja (0,0 = gornji lijevi ugao, 15,15 = donji desni ugao)
void mijenjajDiodu()
{
